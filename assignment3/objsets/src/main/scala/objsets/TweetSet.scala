...
abstract class TweetSet {
   ...
   */
   def union(that: TweetSet): TweetSet = 
    filterAcc(this.contains(_),that.filterAcc(that.contains(_),new Empty))
  ...
  def mostRetAcc(tweet:Tweet):Tweet = tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetAcc(tweet:Tweet):Tweet =
    if (tweet.retweets>elem.retweets) right.mostRetAcc(left.mostRetAcc(tweet))
    else right.mostRetAcc(left.mostRetAcc(elem))
